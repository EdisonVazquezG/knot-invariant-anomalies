...
for setting in SWEEP:
    ...
    patched = source.replace(OUT_LINE, f"OUT = Path({str(run_dir)!r})")
    # Stage 23 may bind arbitrary module-level names. A fresh namespace keeps
    # those assignments out of the sweep's path and aggregation variables.
    run_state = dict(globals())
    run_state.update(__file__=str(STAGE23), OUTPUT_DIR=ROOT_DIR)
    previous_dim = os.environ.get("STAGE23_RESIDUAL_DIM")
    try:
        os.environ["STAGE23_RESIDUAL_DIM"] = str(dim)
        exec(compile(patched, str(STAGE23), "exec"), run_state)
    finally:
        if previous_dim is None:
            os.environ.pop("STAGE23_RESIDUAL_DIM", None)
        else:
            os.environ["STAGE23_RESIDUAL_DIM"] = previous_dim
    ...
    fam = run_state["score_family_summary"].copy()
    fam.insert(0, "residual_dim", label)
    families.append(fam)

    dia = run_state["score_norm_diagnostics"].copy()
    dia.insert(0, "residual_dim", label)
    diagnostics.append(dia)

    sel = run_state["score_selected_ids"].copy()
