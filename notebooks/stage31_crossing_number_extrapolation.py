...
#
# Standalone version: can run from a fresh Colab runtime.
# It rebuilds only the aligned in-memory dataset from the raw invariant files;
# it does NOT rerun paper_run.ipynb or any earlier PCA/null/AE stages.
...
#
# Rebuild only the aligned in-memory dataset from raw files. This does NOT
# rerun paper_run.ipynb, previous PCA/null stages, or autoencoders.
# ---------------------------------------------------------------------------
import sys

# Mount Drive automatically in Colab if needed.
if Path("/content").exists() and not Path("/content/drive/MyDrive").exists():
    try:
        ...
        raise RuntimeError(
            ...
            "Mount Drive manually and rerun this stage."
        ) from exc
...
DEFAULT_PROJECT_DIR = Path(
    (__import__("os").environ.get('KNOT_PROJECT_DIR', '/content/drive/MyDrive/consensus_hardness_refactored') + '')
)
...
# Make the project importable even from a fresh Colab working directory.
for candidate in (PROJECT_DIR, PROJECT_DIR / "notebooks"):
...
try:
    ...
    raise ImportError(
        "Could not import consensus_hardness. Expected the project near:\n"
        f"  {PROJECT_DIR}\n"
        "If your project lives elsewhere, set KNOT_PROJECT_DIR before running."
    ) from exc

if not DATA_DIR.exists():
    raise FileNotFoundError(
        f"Raw knot-data directory not found: {DATA_DIR}\n"
        "Set KNOT_DATA_DIR if your Drive layout differs."
    )
...
if missing:
    ...
    if missing_files:
        raise FileNotFoundError(
            "Missing raw invariant files:\n  "
            + "\n  ".join(str(p) for p in missing_files)
        )

    print("=" * 88)
    print("STAGE 31 — FRESH-RUNTIME DATA BOOTSTRAP")
    print("=" * 88)
    print("Rebuilding the aligned in-memory dataset only.")
    print("This does NOT rerun paper_run.ipynb, previous PCA stages, nulls, or AEs.")
...
expected_views = ("Alexander", "Jones", "HOMFLY-PT", "Theta", "Khovanov")
if set(X_dict) != set(expected_views):
    raise RuntimeError(
        "Unexpected invariant views after alignment: "
        f"{sorted(X_dict.keys())}"
    )

n_rows = len(meta)
if n_rows != CONFIG.universe.expected_n:
    raise RuntimeError(
        f"Aligned universe has {n_rows:,} rows; "
        f"expected {CONFIG.universe.expected_n:,}."
    )

for name in expected_views:
    if np.asarray(X_dict[name]).shape[0] != n_rows:
        raise RuntimeError(f"{name}: matrix row count does not match metadata.")
    if np.asarray(X_dict[name]).shape[1] != len(feature_cols_dict[name]):
        raise RuntimeError(f"{name}: feature-column count does not match matrix width.")

print(f"Aligned universe ready: {n_rows:,} knots.")
print("Views:", ", ".join(expected_views))

OUT = Path(OUTPUT_DIR) / "31_crossing_number_extrapolation"
OUT.mkdir(parents=True, exist_ok=True)

INVARIANTS = expected_views
...
if min(len(train_idx), len(cal_idx), len(test_idx)) == 0:
    raise RuntimeError(
        "Crossing split is empty in at least one partition: "
        f"train<=13={len(train_idx)}, cal14={len(cal_idx)}, test15={len(test_idx)}"
    )

print(
    f"Crossing split: train<=13={len(train_idx):,}, "
    f"cal14={len(cal_idx):,}, test15={len(test_idx):,}"
)
