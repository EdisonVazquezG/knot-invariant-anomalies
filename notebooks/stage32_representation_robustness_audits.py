...
#      support on a reproducible 500-row sample.
#
...
SAMPLE_N = int(os.environ.get("STAGE32_KH_SAMPLE_N", "500"))
...
def locate_theta_source() -> Path:
    ...
    for p in candidates:
        ...
        ncols = len(cols)
        score = abs(ncols - 842)  # 841 features + one ID is the expected source shape.
...
# The archive source is expected to contain one ID column plus 841 coefficient
# coordinates. If exactly 842 columns are present, every non-ID column is a Theta
# coordinate. Otherwise infer numeric coefficient columns from a small sample.
if theta_id_col and len(theta_columns) - 1 == 841:
    theta_features = [c for c in theta_columns if c != theta_id_col]
elif len(theta_columns) == 841:
    theta_features = list(theta_columns)
else:
    # Small-sample dtype inference. Exclude obvious metadata columns.
    ...
    excluded = set(ID_CANDIDATES) | {
        "crossing_number", "crossings", "signature", "s_invariant", "is_alternating"
    }
    theta_features = [
...
for chunk in iter_chunks(KH_PATH, usecols=kh_usecols):
    ...
    if kh_id_col is not None:
        ...
        # Prefer the exact canonical working universe when direct ID matching works.
        if in_canonical.any():
...
sample_out.to_csv(OUT / "khovanov_width_concordance_sample500.csv", index=False)
