...
def _direct_default_data():
    return Path(os.environ.get("KNOT_DATA_DIR", str(_direct_state().get(
        "DATA_DIR", (__import__("os").environ.get('KNOT_DATA_DIR', '/content/drive/MyDrive/Colab Notebooks/data_invariants/Invariants') + '')))))
...
def main():
    ...
    p.add_argument("--notebook", type=Path, default=REPO / "notebooks/archive/paper_run_recorded.ipynb")
    a = p.parse_args()
    ...
    for i, cell in enumerate(nb["cells"]):
        ...
        refs = [token for line in re.findall(r"^%run\s+([^\n]+)", source, re.M)
                for token in shlex.split(line) if token.endswith(".py")]
        outputs = []
        errors = []
        ...
        if re.search(r"#\s*Stage 23[CD]\b", source):
        ...
        if refs:
            for ref in refs:
                ...
                inventory.append(dict(cell_index=i, script=name,
                                      script_present=any((folder / name).is_file() for folder in (REPO / "notebooks", REPO)),
